...
class MouseEditor:
    def __init__(self, master, mouseDB, selected_mouse, mode="edit"):
        self.master = master
    ...
    def edit_mouse_entries(self):
        ...
        # Inform the visualizer that editing has started for plotting red circles
        if self.edit_mouse_var:
            self.set_editing_state(True, self.edit_mouse_var.get('ID'))
        ...
        save_edit_button = tk.Button(self.edit_entry_frame, text="Save Changes", command=self.save_edited_entry)
        ...
        if self.mode == "edit":
            ...
            def update_id_animation():
                if self.reroll_active:
                    ...
                    disp_id.insert(0, mdb_utils.generate_random_id())
                    disp_id.config(state='readonly')
    ...
    def edit_gender_element(self):
        ...
        edit_sex_var = tk.StringVar(self.edit_entry_frame)
        ...
        if self.mode == "edit":
            edit_sex_var.set(self.edit_mouse_var.get('sex', '♂'))
        else:
            edit_sex_var.set("♂ ")

    def edit_toe_element(self):
        tk.Label(self.edit_entry_frame, text="Toe:").grid(row=3, column=0, padx=5, pady=5, sticky="w")
        edit_toe_entry = tk.Entry(self.edit_entry_frame)
        edit_toe_entry.grid(row=3, column=1, columnspan=2, padx=5, pady=5, sticky="ew")

        if self.mode == "edit":
            edit_toe_entry.insert(0, self.edit_mouse_var.get('toe', '').replace('toe', ''))

    def edit_genotype_element(self):
        tk.Label(self.edit_entry_frame, text="Genotype:").grid(row=4, column=0, padx=5, pady=5, sticky="w")
        edit_genotype_entry = tk.Entry(self.edit_entry_frame)
        edit_genotype_entry.grid(row=4, column=1, columnspan=2, padx=5, pady=5, sticky="ew")

        if self.mode == "edit":
            edit_genotype_entry.insert(0, self.edit_mouse_var.get('genotype', ''))

    def edit_birthdate_element(self):
        tk.Label(self.edit_entry_frame, text="Birth Date:").grid(row=5, column=0, padx=5, pady=5, sticky="w")
        edit_birthdate_entry = tk.Entry(self.edit_entry_frame)
        edit_birthdate_entry.grid(row=5, column=1, columnspan=2, padx=5, pady=5, sticky="ew")

        if self.mode == "edit":
            birth_date = self.edit_mouse_var.get('birthDate', '')
            birth_date_str = mdb_utils.convert_dates_to_string(birth_date)
            edit_birthdate_entry.insert(0, birth_date_str)

    def edit_breeddate_element(self):
        tk.Label(self.edit_entry_frame, text="Breed Date:").grid(row=6, column=0, padx=5, pady=5, sticky="w")
        edit_breeddate_entry = tk.Entry(self.edit_entry_frame)
        edit_breeddate_entry.grid(row=6, column=1, columnspan=2, padx=5, pady=5, sticky="ew")

        if self.mode == "edit":
            breed_date = self.edit_mouse_var.get('breedDate', '')
            breed_date_str = mdb_utils.convert_dates_to_string(breed_date)
            edit_breeddate_entry.insert(0, breed_date_str)
        else:
            edit_breeddate_entry.insert(0, "Non Applicable")
            edit_breeddate_entry.config(state='readonly')
