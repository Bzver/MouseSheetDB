...
def mice_changelog(old_dict, new_dict, output_path):
    try:
        ...
        df_added, df_changed, df_manual = organize_changelog_df(added_entries, changed_entries, COMPARE_COLUMNS)
...
def load_changelog_add(changelog_df, mice_dict, changes_applied_count, exception_entries):
    mice_added_count = 0  # Initialize mice_added_count
...
def find_changes_for_changelog(old_dict, new_dict, fields_to_compare=None, check_only=False):
    ...
    changed = [
        ...
        any(e.get(f) != old_dict.get(e["ID"], {}).get(f) for f in fields_to_compare)
    ]
...
def organize_changelog_df(added_entries, changed_entries, fields_to_compare):
    ...
    if not df_added.empty:
        df_added = mut.df_date_col_formatter(df_added, DATE_COLUMNS)
    if not df_changed.empty:
        df_changed = mut.df_date_col_formatter(df_changed, DATE_COLUMNS)
    if not df_manual.empty:
