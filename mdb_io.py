...
def write_processed_data_to_excel(excel_file, processed_data):
    # Create a new dictionary to hold mice that will be written to Excel. Mice in "Death Row" will be placed into Memorial category.
    ...
    df_format = mut.df_date_col_formatter(df_sorted, date_cols)

    df_final = mut.cleanup_optional_cols(df_format)
    df_filled = df_final.fillna("-")

    try:
        with pd.ExcelWriter(excel_file, engine="xlsxwriter") as writer:
            df_filled.to_excel(writer, sheet_name="MDb", index=False)
...
def organize_changelog_df(added_entries, changed_entries, fields_to_compare=None):
    ...
    date_cols = ["birthDate", "breedDate"]
    if not df_added.empty:
        df_added = mut.df_date_col_formatter(df_added, date_cols)
    if not df_changed.empty:
        df_changed = mut.df_date_col_formatter(df_changed, date_cols)
    if not df_manual.empty:
        df_manual = mut.df_date_col_formatter(df_manual, ["birthDate"])
